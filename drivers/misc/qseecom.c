				    ...
				    void __user *argp)
{
	...
	if (__validate_send_service_cmd_inputs(data, &req)) {
		sktrc(SKTRC_HASH(0, 2, 1, 3),
		      "validate_send_service_cmd_inputs (%ld)", EINVAL);
		return -EINVAL;
	}
			      ...
			      struct qseecom_send_cmd_req *req)
{
	...
	if (!found_app) {
		sktrc(SKTRC_HASH(0, 2, 1, 7), "app_id %d (%s) is not found\n",
		      data->client.app_id, (char *)data->client.app_name);
		return -ENOENT;
	}
	...
	if (qseecom.qsee_version < QSEE_VERSION_40) {
		...
		sktrc(SKTRC_HASH(0, 0, 1, 7),
		      "Request pointer physical address: %p",
		      (void*)&send_data_req.req_ptr);
		sktrc(SKTRC_HASH(0, 0, 1, 7),
		      "Response pointer physical address: %p",
...
/* Comment out below */
		if (cmd_buf!=NULL) {
			...
			sktrc(SKTRC_HASH(0, 0, 1, 7),
			      "qseecom_send_cmd(32): CONTENT OF SEND CMD: %s",
			      buffer);
			kfree(buffer);
		}
/* Comment out above */
		...
		if ((data->client.app_arch == ELFCLASS32) &&
			...
			sktrc(SKTRC_HASH(0, 2, 1, 7),
			      "32bit app %s PA exceeds 4G: req_ptr=%llx, req_len=%x, rsp_ptr=%llx, rsp_len=%x (%ld)",
			      data->client.app_name,
			      send_data_req_64bit.req_ptr,
			      send_data_req_64bit.req_len,
			      send_data_req_64bit.rsp_ptr,
			      send_data_req_64bit.rsp_len, EINVAL);
			return -EFAULT;
		}
		...
		sktrc(SKTRC_HASH(0, 0, 1, 7),
		      "Request64 pointer physical address: %p",
		      (void*)&send_data_req_64bit.req_ptr);
		sktrc(SKTRC_HASH(0, 0, 1, 7),
		      "Response64 pointer physical address: %p",
...
/* Comment out below */
		if (cmd_buf!=NULL)
		{
			...
			sktrc(SKTRC_HASH(0, 0, 1, 7),
			      "qseecom_send_cmd(64): CONTENT OF SEND CMD: %s",
			      buffer);
			kfree(buffer);
		}
/* Comment out above */
...
fail2:
	...
	if (ret) {
		sktrc(SKTRC_HASH(0, 2, 1, 7),
		      "cache operation failed %d (fail2)", ret);
		return ret;
	}
	...
	if (ret) {
		sktrc(SKTRC_HASH(0, 2, 1, 7),
		      "scm_call() failed with err: %d (app_id = %d)", ret,
		      data->client.app_id);
		return ret;
	}
	...
	if (qseecom.qsee_reentrancy_support) {
		...
		if (resp.result == QSEOS_RESULT_INCOMPLETE) {
			...
			if (ret) {
				sktrc(SKTRC_HASH(0, 2, 1, 7),
				      "process_incomplete_cmd failed err: %d",
				      ret);
				return ret;
			}
		} else {
			if (resp.result != QSEOS_RESULT_SUCCESS) {
				sktrc(SKTRC_HASH(0, 2, 1, 7),
				      "Response result %d not supported",
				      resp.result);
				ret = -EINVAL;
			}
		}
	...
	ret = msm_ion_do_cache_op(qseecom.ion_clnt, data->client.ihandle,
				  ...
				  ION_IOC_INV_CACHES);
	if (ret)
		sktrc(SKTRC_HASH(0, 2, 1, 7), "cache operation failed %d (ret)",
		      ret);
	return ret;
}
...
static int qseecom_send_cmd(struct qseecom_dev_handle *data, void __user *argp)
{
	...
	if (ret) {
		sktrc(SKTRC_HASH(0, 2, 1, 4), "copy_from_user failed");
		return ret;
	}

	if (__validate_send_cmd_inputs(data, &req)) {
		sktrc(SKTRC_HASH(0, 2, 1, 4), "validate_send_cmd_inputs (%ld)",
		      EINVAL);
		return -EINVAL;
	}
				    ...
				    bool is_64bit_addr)
{
	...
	ret = copy_from_user(&req, argp, sizeof(req));
	if (ret) {
		sktrc(SKTRC_HASH(0, 2, 1, 5), "copy_from_user failed\n");
		return ret;
	}
	...
	if (__validate_send_cmd_inputs(data, &send_cmd_req)) {
		sktrc(SKTRC_HASH(0, 2, 1, 5), "validate_send_cmd_inputs (%ld)",
		      EINVAL);
		return -EINVAL;
	}
			 ...
			 uint32_t sbuf_len, void *resp_buf, uint32_t rbuf_len)
{
	...
	if (__validate_send_cmd_inputs(data, &req)) {
		sktrc(SKTRC_HASH(0, 2, 1, 6), "validate_send_cmd_inputs (%ld)",
		      EINVAL);
		return -EINVAL;
	}
					     ...
					     struct qseecom_registered_listener_list *this_lstnr)
{
	...
	if (!data || !resp || !this_lstnr) {
		sktrc(SKTRC_HASH(0, 2, 1, 12),
		      "listener handle or resp msg is null (%ld)", EINVAL);
		return -EINVAL;
	}

	if (resp->resp_buf_ptr == NULL) {
		sktrc(SKTRC_HASH(0, 2, 1, 12), "resp buffer is null (%ld)",
		      EINVAL);
		return -EINVAL;
	}
	...
	if ((resp->resp_len == 0) ||
		...
		sktrc(SKTRC_HASH(0, 2, 1, 12),
		      "resp buf length %d not valid (%ld)", resp->resp_len,
		      EINVAL);
		return -EINVAL;
	}
	...
	if ((uintptr_t)resp->resp_buf_ptr > (ULONG_MAX - resp->resp_len)) {
		sktrc(SKTRC_HASH(0, 2, 1, 12),
		      "Integer overflow in resp_len & resp_buf (%ld)", EINVAL);
		return -EINVAL;
	}
	if ((uintptr_t)this_lstnr->user_virt_sb_base >
	    (ULONG_MAX - this_lstnr->sb_length)) {
		sktrc(SKTRC_HASH(0, 2, 1, 12),
		      "Integer overflow in user_virt_sb_base & sb_length (%ld)",
		      EINVAL);
		return -EINVAL;
	}
	/* validate resp buf */
	if (((uintptr_t)resp->resp_buf_ptr <
	    ...
	    (((uintptr_t)resp->resp_buf_ptr + resp->resp_len) >
	     ((uintptr_t)this_lstnr->user_virt_sb_base +
	      this_lstnr->sb_length))) {
		sktrc(SKTRC_HASH(0, 2, 1, 12),
		      "resp buf is out of shared buffer region (%ld)", EINVAL);
		return -EINVAL;
	}
					  ...
					  void __user *argp)
{
	...
	ret = copy_from_user(&req, argp,
			     sizeof(struct qseecom_qteec_modfd_req));
	if (ret) {
		sktrc(SKTRC_HASH(0, 2, 1, 13), "copy_from_user failed");
		return ret;
	}
	...
	spin_unlock_irqrestore(&qseecom.registered_app_list_lock, flags);
	if (!found_app) {
		sktrc(SKTRC_HASH(0, 2, 1, 13),
		      "app_id %d (%s) is not found (%ld)",
		      data->client.app_id, (char *)data->client.app_name,
		      ENOENT);
		return -ENOENT;
	}
	...
	ret = msm_ion_do_cache_op(qseecom.ion_clnt, data->client.ihandle,
				  ...
				  ION_IOC_CLEAN_INV_CACHES);
	if (ret) {
		sktrc(SKTRC_HASH(0, 2, 1, 13),"cache operation failed %d", ret);
		return ret;
	}
	...
	if (ret) {
		sktrc(SKTRC_HASH(0, 2, 1, 13),
		      "scm_call() failed with err: %d (app_id = %d)",
		      ret, data->client.app_id);
		return ret;
	}
	...
	ret = msm_ion_do_cache_op(qseecom.ion_clnt, data->client.ihandle,
				  ...
				  ION_IOC_INV_CACHES);
	if (ret) {
		sktrc(SKTRC_HASH(0, 2, 1, 13), "cache operation failed %d",
		      ret);
		return ret;
	}
	return 0;
...
static int send_atomic_scm(void __user *argp)
{
	...
	ret = copy_from_user(&req, argp, sizeof(req));
	if (ret) {
		sktrc(SKTRC_HASH(0, 2, 1, 14), "copy_from_user failed");
		return ret;
	}
	sktrc(SKTRC_HASH(0, 0, 1, 14), "Going to send atomic SCM request");
	if (req.num_args == 1) {
		...
		sktrc(SKTRC_HASH(0, 0, 1, 14),
		      "result = 0x%x, resp_type = 0x%x, data = 0x%x",
	...
	sktrc(SKTRC_HASH(0, 0, 1, 14), "Finished raw SCM request");
	return ret;
}
...
static int send_raw_scm(void __user *argp)
{
	...
	ret = copy_from_user(&req, argp, sizeof(req));
	if (ret) {
		sktrc(SKTRC_HASH(0, 2, 1, 15), "copy_from_user failed");
		return ret;
	}
	sktrc(SKTRC_HASH(0, 0, 1, 15),
	      "Going to send raw SCM request (no remap error!)");
	ret = scm_call_no_remap_error(req.svc_id, req.cmd_id, req.cmd_req_buf, req.cmd_req_len, req.resp_buf, req.resp_len);
	sktrc(SKTRC_HASH(0, 0, 1, 15), "Finished raw SCM request");
	return ret;
}
...
long qseecom_ioctl(struct file *file, unsigned cmd, unsigned long arg)
{
	...
	if (!data) {
		sktrc(SKTRC_HASH(0, 2, 1, 11),
		      "Invalid/uninitialized device handle (%ld)", EINVAL);
		return -EINVAL;
	}

	if (data->abort) {
		sktrc(SKTRC_HASH(0, 2, 1, 11), "Aborting qseecom driver (%ld)",
		      ENODEV);
		return -ENODEV;
	}
	...
	case QSEECOM_IOCTL_SEND_RAW_SCM: {
		...
		sktrc(SKTRC_HASH(0, 0, 1, 11), "send_raw_scm");
		ret = send_raw_scm(argp);
		atomic_dec(&data->ioctl_count);
		wake_up_all(&data->abort_wq);
		if (ret && (ret != -ERESTARTSYS))
			sktrc(SKTRC_HASH(0, 2, 1, 11),
			      "failed ioctl_send_raw: %d", ret);
		break;
	}
	case QSEECOM_IOCTL_SEND_ATOMIC_SCM: {
		atomic_inc(&data->ioctl_count);
		sktrc(SKTRC_HASH(0, 0, 1, 11), "send_atomic_scm");
		ret = send_atomic_scm(argp);
		atomic_dec(&data->ioctl_count);
		wake_up_all(&data->abort_wq);
		if (ret && (ret != -ERESTARTSYS))
			sktrc(SKTRC_HASH(0, 2, 1, 11),
			      "failed ioctl_send_atomic: %d", ret);
		break;
	}
	case QSEECOM_IOCTL_REGISTER_LISTENER_REQ: {
		if (data->type != QSEECOM_GENERIC) {
			sktrc(SKTRC_HASH(0, 2, 1, 11),
			      "reg lstnr req: invalid handle (%d)",
			      data->type);
			ret = -EINVAL;
			break;
		}
		sktrc(SKTRC_HASH(0, 0, 1, 11), "ioctl register_listener_req()");
		mutex_lock(&app_access_lock);
		atomic_inc(&data->ioctl_count);
		data->type = QSEECOM_LISTENER_SERVICE;
		...
		if (ret)
			sktrc(SKTRC_HASH(0, 2, 1, 11),
			      "failed qseecom_register_listener: %d",
			      ret);
		break;
	}
	case QSEECOM_IOCTL_UNREGISTER_LISTENER_REQ: {
		if ((data->listener.id == 0) ||
		    (data->type != QSEECOM_LISTENER_SERVICE)) {
			sktrc(SKTRC_HASH(0, 2, 1, 11),
			      "unreg lstnr req: invalid handle (%d) lid(%d) (%ld)",
			      data->type, data->listener.id, EINVAL);
			ret = -EINVAL;
			break;
		}
		sktrc(SKTRC_HASH(0, 0, 1, 11),
		      "ioctl unregister_listener_req()");
		mutex_lock(&app_access_lock);
		atomic_inc(&data->ioctl_count);
		...
		if (ret)
			sktrc(SKTRC_HASH(0, 2, 1, 11),
			      "failed qseecom_unregister_listener: %d",
			      ret);
		break;
	}
	case QSEECOM_IOCTL_SEND_CMD_REQ: {
		if ((data->client.app_id == 0) ||
		    (data->type != QSEECOM_CLIENT_APP)) {
			sktrc(SKTRC_HASH(0, 2, 1, 11),
			      "send cmd req: invalid handle (%d) app_id(%d)",
	...
	case QSEECOM_IOCTL_SEND_MODFD_CMD_64_REQ: {
		if ((data->client.app_id == 0) ||
		    (data->type != QSEECOM_CLIENT_APP)) {
			sktrc(SKTRC_HASH(0, 2, 1, 11),
			      "send mdfd cmd: invalid handle (%d) appid(%d)",
			      data->type, data->client.app_id);
			ret = -EINVAL;
			break;
		}
	...
	case QSEECOM_IOCTL_RECEIVE_REQ: {
		if ((data->listener.id == 0) ||
		    (data->type != QSEECOM_LISTENER_SERVICE)) {
			sktrc(SKTRC_HASH(0, 2, 1, 11),
			      "receive req: invalid handle (%d), lid(%d)",
		...
		if (ret && (ret != -ERESTARTSYS))
			sktrc(SKTRC_HASH(0, 2, 1, 11),
			      "failed qseecom_receive_req: %d", ret);
		break;
	}
	case QSEECOM_IOCTL_SEND_RESP_REQ: {
		if ((data->listener.id == 0) ||
		    (data->type != QSEECOM_LISTENER_SERVICE)) {
			sktrc(SKTRC_HASH(0, 2, 1, 11),
			      "send resp req: invalid handle (%d), lid(%d)",
			      data->type, data->listener.id);
			ret = -EINVAL;
			break;
		}
		...
		if (ret)
			sktrc(SKTRC_HASH(0, 2, 1, 11),
			      "failed qseecom_send_resp: %d", ret);
		break;
	}
	case QSEECOM_IOCTL_SET_MEM_PARAM_REQ: {
		if ((data->type != QSEECOM_CLIENT_APP) &&
		    ...
		    (data->type != QSEECOM_SECURE_SERVICE)) {
			sktrc(SKTRC_HASH(0, 2, 1, 11),
			      "set mem param req: invalid handle (%d)",
			      data->type);
			ret = -EINVAL;
			break;
		}
		sktrc(SKTRC_HASH(0, 0, 1, 11),
		      "SET_MEM_PARAM: qseecom addr = 0x%pK", data);
		mutex_lock(&app_access_lock);
		atomic_inc(&data->ioctl_count);
		...
		if (ret)
			sktrc(SKTRC_HASH(0, 2, 1, 11),
			      "failed Qqseecom_set_mem_param request: %d\n",
			      ret);
		break;
	}
	case QSEECOM_IOCTL_LOAD_APP_REQ: {
		...
		data->type = QSEECOM_CLIENT_APP;
		sktrc(SKTRC_HASH(0, 0, 1, 11),
		      "LOAD_APP_REQ: qseecom_addr = 0x%pK", data);
		mutex_lock(&app_access_lock);
		atomic_inc(&data->ioctl_count);
	...
	case QSEECOM_IOCTL_UNLOAD_APP_REQ: {
		if ((data->client.app_id == 0) ||
		    (data->type != QSEECOM_CLIENT_APP)) {
			sktrc(SKTRC_HASH(0, 2, 1, 11),
			      "unload app req:invalid handle(%d) app_id(%d)",
			      data->type, data->client.app_id);
			ret = -EINVAL;
			break;
		}
		sktrc(SKTRC_HASH(0, 0, 1, 11),
		      "UNLOAD_APP: qseecom_addr = 0x%pK", data);
		mutex_lock(&app_access_lock);
		atomic_inc(&data->ioctl_count);
		ret = qseecom_unload_app(data, false);
		atomic_dec(&data->ioctl_count);
		...
		if (ret)
			sktrc(SKTRC_HASH(0, 2, 1, 11),
			      "failed unload_app request: %d", ret);
		break;
	}
	case QSEECOM_IOCTL_GET_QSEOS_VERSION_REQ: {
		...
		if (ret)
			sktrc(SKTRC_HASH(0, 0, 1, 11),
			      "qseecom_get_qseos_version: %d", ret);
		atomic_dec(&data->ioctl_count);
		break;
	}
	case QSEECOM_IOCTL_PERF_ENABLE_REQ:{
		if ((data->type != QSEECOM_GENERIC) &&
		    (data->type != QSEECOM_CLIENT_APP)) {
			sktrc(SKTRC_HASH(0, 2, 1, 11),
			      "perf enable req: invalid handle (%d)",
			      data->type);
			ret = -EINVAL;
			break;
		}
		if ((data->type == QSEECOM_CLIENT_APP) &&
			...
			sktrc(SKTRC_HASH(0, 2, 1, 11),
			      "perf enable req:invalid handle(%d) appid(%d)",
			      data->type, data->client.app_id);
			ret = -EINVAL;
			break;
		}
	...
	case QSEECOM_IOCTL_PERF_DISABLE_REQ:{
		if ((data->type != QSEECOM_SECURE_SERVICE) &&
		    (data->type != QSEECOM_CLIENT_APP)) {
			sktrc(SKTRC_HASH(0, 2, 1, 11),
			      "perf disable req: invalid handle (%d)",
			      data->type);
			ret = -EINVAL;
			break;
		}
		if ((data->type == QSEECOM_CLIENT_APP) &&
		    (data->client.app_id == 0)) {
			sktrc(SKTRC_HASH(0, 2, 1, 11),
			      "perf disable: invalid handle (%d)app_id(%d)",
			      data->type, data->client.app_id);
			ret = -EINVAL;
			break;
		}
	...
	case QSEECOM_IOCTL_SET_BUS_SCALING_REQ: {
		...
		if ((data->client.app_id == 0) ||
		    (data->type != QSEECOM_CLIENT_APP)) {
			sktrc(SKTRC_HASH(0, 2, 1, 11),
			      "set bus scale: invalid handle (%d) appid(%d)",
			      data->type, data->client.app_id);
			ret = -EINVAL;
			break;
		}
	...
	case QSEECOM_IOCTL_LOAD_EXTERNAL_ELF_REQ: {
		if (data->type != QSEECOM_GENERIC) {
			sktrc(SKTRC_HASH(0, 2, 1, 11),
			      "load ext elf req: invalid client handle (%d)",
			      data->type);
			ret = -EINVAL;
			break;
		}
		...
		if (ret)
			sktrc(SKTRC_HASH(0, 2, 1, 11),
			      "failed load_external_elf request: %d",
			      ret);
		break;
	}
	case QSEECOM_IOCTL_UNLOAD_EXTERNAL_ELF_REQ: {
		if (data->type != QSEECOM_UNAVAILABLE_CLIENT_APP) {
			sktrc(SKTRC_HASH(0, 2, 1, 11),
			      "unload ext elf req: invalid handle (%d)",
			      data->type);
			ret = -EINVAL;
			break;
		}
	...
	case QSEECOM_IOCTL_APP_LOADED_QUERY_REQ: {
		...
		atomic_inc(&data->ioctl_count);
		sktrc(SKTRC_HASH(0, 0, 1, 11),
		      "APP_LOAD_QUERY: qseecom_addr = 0x%pK", data);
	...
	case QSEECOM_IOCTL_SEND_CMD_SERVICE_REQ: {
		if (data->type != QSEECOM_GENERIC) {
			sktrc(SKTRC_HASH(0, 2, 1, 11),
			      "send cmd svc req: invalid handle (%d)",
			      data->type);
			ret = -EINVAL;
			break;
		}
		...
		if (qseecom.qsee_version < QSEE_VERSION_03) {
			sktrc(SKTRC_HASH(0, 2, 1, 11),
			      "SEND_CMD_SERVICE_REQ: Invalid qsee ver %u",
	...
	case QSEECOM_IOCTL_CREATE_KEY_REQ: {
		if (!(qseecom.support_pfe || qseecom.support_fde))
			sktrc(SKTRC_HASH(0, 0, 1, 11),
			      "Features requiring key init not supported");
		if (data->type != QSEECOM_GENERIC) {
			sktrc(SKTRC_HASH(0, 2, 1, 11),
			      "create key req: invalid handle (%d)",
			      data->type);
			ret = -EINVAL;
			break;
		}
		if (qseecom.qsee_version < QSEE_VERSION_05) {
			sktrc(SKTRC_HASH(0, 2, 1, 11),
			      "Create Key feature unsupported: qsee ver %u",
			      qseecom.qsee_version);
			return -EINVAL;
		}
		data->released = true;
		...
		if (ret)
			sktrc(SKTRC_HASH(0, 2, 1, 11),
			      "failed to create encryption key: %d",
			      ret);
	...
	case QSEECOM_IOCTL_WIPE_KEY_REQ: {
		...
		if (qseecom.qsee_version < QSEE_VERSION_05) {
			sktrc(SKTRC_HASH(0, 2, 1, 11),
			      "Wipe Key feature unsupported in qsee ver %u",
			      qseecom.qsee_version);
			return -EINVAL;
		}
		data->released = true;
		...
		if (ret)
			sktrc(SKTRC_HASH(0, 2, 1, 11),
			      "failed to wipe encryption key: %d",
			      ret);
		atomic_dec(&data->ioctl_count);
		mutex_unlock(&app_access_lock);
		break;
	...
	case QSEECOM_IOCTL_UPDATE_KEY_USER_INFO_REQ: {
		...
		if (data->type != QSEECOM_GENERIC) {
			sktrc(SKTRC_HASH(0, 2, 1, 11),
			      "update key req: invalid handle (%d)",
			      data->type);
			ret = -EINVAL;
			break;
		}
		if (qseecom.qsee_version < QSEE_VERSION_05) {
			sktrc(SKTRC_HASH(0, 2, 1, 11),
			      "Update Key feature unsupported in qsee ver %u",
			      qseecom.qsee_version);
			return -EINVAL;
		}
		data->released = true;
		...
		if (ret)
			sktrc(SKTRC_HASH(0, 2, 1, 11),
			      "failed to update key user info: %d",
			      ret);
		atomic_dec(&data->ioctl_count);
		mutex_unlock(&app_access_lock);
		break;
	...
	case QSEECOM_IOCTL_SAVE_PARTITION_HASH_REQ: {
		if (data->type != QSEECOM_GENERIC) {
			sktrc(SKTRC_HASH(0, 2, 1, 11),
			      "save part hash req: invalid handle (%d)",
			      data->type);
			ret = -EINVAL;
			break;
		}
	...
	case QSEECOM_IOCTL_IS_ES_ACTIVATED_REQ: {
		if (data->type != QSEECOM_GENERIC) {
			sktrc(SKTRC_HASH(0, 2, 1, 11),
			      "ES activated req: invalid handle (%d)",
			      data->type);
			ret = -EINVAL;
			break;
		}
	...
	case QSEECOM_IOCTL_MDTP_CIPHER_DIP_REQ: {
		if (data->type != QSEECOM_GENERIC) {
			sktrc(SKTRC_HASH(0, 2, 1, 11),
			      "MDTP cipher DIP req: invalid handle (%d)",
			      data->type);
			ret = -EINVAL;
			break;
		}
	...
	case QSEECOM_IOCTL_SEND_MODFD_RESP_64: {
		...
		if (ret)
			sktrc(SKTRC_HASH(0, 2, 1, 11),
			      "failed qseecom_send_mod_resp: %d", ret);
		__qseecom_clean_data_sglistinfo(data);
		break;
	}
	case QSEECOM_QTEEC_IOCTL_OPEN_SESSION_REQ: {
		if ((data->client.app_id == 0) ||
		    (data->type != QSEECOM_CLIENT_APP)) {
			sktrc(SKTRC_HASH(0, 2, 1, 11),
			      "Open session: invalid handle (%d) appid(%d)",
			      data->type, data->client.app_id);
			ret = -EINVAL;
			break;
		}
		if (qseecom.qsee_version < QSEE_VERSION_40) {
			sktrc(SKTRC_HASH(0, 2, 1, 11),
			      "GP feature unsupported: qsee ver %u",
			      qseecom.qsee_version);
			return -EINVAL;
		}
		/* Only one client allowed here at a time */
	...
	case QSEECOM_QTEEC_IOCTL_CLOSE_SESSION_REQ: {
		if ((data->client.app_id == 0) ||
		    (data->type != QSEECOM_CLIENT_APP)) {
			sktrc(SKTRC_HASH(0, 2, 1, 11),
			      "Close session: invalid handle (%d) appid(%d)",
			      data->type, data->client.app_id);
			ret = -EINVAL;
			break;
		}
		...
		if (ret)
			sktrc(SKTRC_HASH(0, 2, 1, 11),
			      "failed close_session_cmd: %d", ret);
		break;
	}
	case QSEECOM_QTEEC_IOCTL_INVOKE_MODFD_CMD_REQ: {
		if ((data->client.app_id == 0) ||
		    (data->type != QSEECOM_CLIENT_APP)) {
			sktrc(SKTRC_HASH(0, 2, 1, 11),
			      "Invoke cmd: invalid handle (%d) appid(%d)",
			      data->type, data->client.app_id);
			ret = -EINVAL;
			break;
		}
		...
		mutex_unlock(&app_access_lock);
		if (ret)
			sktrc(SKTRC_HASH(0, 2, 1, 11), "failed Invoke cmd: %d",
			      ret);
		__qseecom_clean_data_sglistinfo(data);
		break;
	}
	case QSEECOM_QTEEC_IOCTL_REQUEST_CANCELLATION_REQ: {
		if ((data->client.app_id == 0) ||
		    (data->type != QSEECOM_CLIENT_APP)) {
			sktrc(SKTRC_HASH(0, 2, 1, 11),
			      "Cancel req: invalid handle (%d) appid(%d)",
			      data->type, data->client.app_id);
			ret = -EINVAL;
			break;
		}
		...
		if (ret)
			sktrc(SKTRC_HASH(0, 2, 1, 11),
			      "failed request_cancellation: %d", ret);
		break;
	}
	case QSEECOM_IOCTL_GET_CE_PIPE_INFO: {
		...
		if (ret)
			sktrc(SKTRC_HASH(0, 2, 1, 11),
			      "failed get fde ce pipe info: %d", ret);
		atomic_dec(&data->ioctl_count);
		break;
	}
	...
	default:
		sktrc(SKTRC_HASH(0, 2, 1, 11), "Invalid IOCTL: 0x%x", cmd);
		return -EINVAL;
	}
	return ret;
}
...
{
	sktrc(SKTRC_HASH(0, 1, 1, 1), "QSEECOM loaded!");
...
static void qseecom_exit(void)
{
	...
	sktrc(SKTRC_HASH(0, 1, 1, 2), "QSEECOM unloaded!");
}
